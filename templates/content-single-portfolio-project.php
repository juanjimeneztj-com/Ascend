...
global $post;
	...
	$juanjimeneztj_project = get_post_meta( $post->ID, '_kad_ppost_type', true );
	...
	$imgwidth 	= get_post_meta( $post->ID, '_kad_portfolio_slider_width', true );
   	if (!empty($imgheight)) {
		...
		$slideheight = apply_filters('juanjimeneztj_single_portfolio_image_height', null); 
	} 
	if (!empty($imgwidth)) {
		$slidewidth = $imgwidth;
	} else {
		$slidewidth = juanjimeneztj_portfolio_slider_width();
		$slidewidth = apply_filters('juanjimeneztj_single_portfolio_image_width', $slidewidth); 
	}
    if(empty($juanjimeneztj_project)) {
    	$juanjimeneztj_project = 'image';
    }

    if ($juanjimeneztj_project == 'flex') { 
    ...
    } else if ($juanjimeneztj_project == 'carouselslider') { 
    ...
    } else if ($juanjimeneztj_project == 'thumbslider') { 
    ...
    } else if ($juanjimeneztj_project == 'imagegrid') { 
    ...
    } else if ($juanjimeneztj_project == 'collage') { 
    ...
    } else if ($juanjimeneztj_project == 'video') { 
    ...
    } else if ($juanjimeneztj_project == 'image') {
        if (has_post_thumbnail( $post->ID ) ) {          
            $image_id = get_post_thumbnail_id();
            $img = juanjimeneztj_get_image_array($slidewidth, $slideheight, true, null, get_the_title(), $image_id, false);
    ...
    }  else if ($juanjimeneztj_project == 'imgcarousel') { ?>
                    ...
                    if(!empty($image_gallery)) {
                        ...
                        if ($attachments) {
                            foreach ($attachments as $attachment) {
                                ...
                                $img = juanjimeneztj_get_image_array(null, $slideheight, true, null, $alt, $attachment, false);
                                if( juanjimeneztj_lazy_load_filter() ) {
                                    $image_src_output = 'src="data:image/gif;base64,R0lGODdhAQABAPAAAP///wAAACwAAAAAAQABAEACAkQBADs=" data-lazy-src="'.esc_url($img['src']).'" '; 
                                } else {
                                    $image_src_output = 'src="'.esc_url($img['src']).'"'; 
