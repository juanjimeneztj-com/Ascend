...
// Don't duplicate me!
if (!class_exists('ReduxFramework_juanjimeneztj_slides')) {
     ...
     * @since       1.0.0
     */
    class ReduxFramework_juanjimeneztj_slides {
        ...
        public function render() {
             ...
             $defaults = array(
                ...
                'content_title' => __ ( 'Slide', 'juanjimeneztj' )
            );

             $this->field = wp_parse_args ( $this->field, $defaults );

           echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'juanjimeneztj' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                   ...
                   if ( empty ( $slide[ 'thumb' ] ) && !empty ( $slide[ 'attachment_id' ] ) ) {
                    ...
                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'juanjimeneztj' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . esc_attr($slide[ 'attachment_id' ]) . '">' . __ ( 'Remove', 'juanjimeneztj' ) . '</span>';

                    echo '</div>' . "\n";

                    echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="redux-slides-list">';

                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-url_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . $x . '][url]" value="' . esc_attr($slide['url']) . '" class="full-text upload" placeholder="'.__('URL', 'juanjimeneztj').'" /></li>';
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-title_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . $x . '][title]" value="' . esc_attr($slide['title']) . '" placeholder="'.__('Title', 'juanjimeneztj').'" class="full-text slide-title" /></li>';
                    echo '<li><textarea name="' . $this->field['name'] . '[' . $x . '][description]" id="' . $this->field['id'] . '-description_' . $x . '" placeholder="'.__('Subtitle', 'juanjimeneztj').'" class="large-text" rows="6">' . esc_attr($slide['description']) . '</textarea></li>';
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-link_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . $x . '][link]" value="' . esc_attr($slide['link']) . '" placeholder="'.__('Slide Link', 'juanjimeneztj').'" class="full-text" /></li>';
                    ...
                    echo ' '.__('Open Link in New Tab/Window', 'juanjimeneztj'). '</label></li>';
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __('Delete Slide', 'juanjimeneztj') . '</a></li>';
            ...
            if ($x == 0) {
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . esc_attr($this->parent->args[ 'opt_name' ]) . '[' . esc_attr($this->field[ 'id' ]) . '][attachment_id]">' . __ ( 'Remove', 'juanjimeneztj' ) . '</span>';

                echo '</div>' . "\n";

                echo '<ul id="' . esc_attr($this->field['id']) . '-ul" class="redux-slides-list">';
                echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-url_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . $x . '][url]" value="" class="full-text upload" placeholder="'.__( 'URL', 'juanjimeneztj' ).'" /></li>';
                echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-title_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . $x . '][title]" value="" placeholder="'.__( 'Title', 'juanjimeneztj' ).'" class="full-text slide-title" /></li>';
                echo '<li><textarea name="' . esc_attr($this->field['name']) . '[' . $x . '][description]" id="' . esc_attr($this->field['id']) . '-description_' . $x . '" placeholder="'.__( 'Subtitle', 'juanjimeneztj' ).'" class="large-text" rows="6"></textarea></li>';
                echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-link_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . $x . '][link]" value="" placeholder="'.__( 'Slide Link', 'juanjimeneztj' ).'" class="full-text" /></li>';
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add2 kad_redux-slides-add button-primary" rel-id="' . esc_attr($this->field[ 'id' ]) . '-ul" rel-name="' . esc_attr($this->field[ 'name' ]) . '[title][]">' . sprintf ( __ ( 'Add %s', 'juanjimeneztj' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
         ...
         public function enqueue () {
            ...
            wp_enqueue_script (
                'juanjimeneztj-field-slides-js', 
                get_template_directory_uri() . '/themeoptions/options/extensions/juanjimeneztj_slides/juanjimeneztj_slides/field_juanjimeneztj_slides' . Redux_Functions::isMin () . '.js', 
            ...
            wp_enqueue_style (
                'juanjimeneztj-field-slides-css', 
                get_template_directory_uri() . '/themeoptions/options/extensions/juanjimeneztj_slides/juanjimeneztj_slides/field_juanjimeneztj_slides.css', 
                time (), 
                true
            );
        }       
