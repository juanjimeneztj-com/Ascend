 ...
 */
function juanjimeneztj_kadence_toolkit_notice() {
	...
	if ( ! isset( $installed_plugins['virtue-toolkit/virtue_toolkit.php'] ) ) {
		$button_label = esc_html__( 'Install Kadence Toolkit', 'juanjimeneztj' );
		$data_action  = 'install';
	} elseif ( ! juanjimeneztj_Plugin_Check::active_check( 'virtue-toolkit/virtue_toolkit.php' ) ) {
		$button_label = esc_html__( 'Activate Kadence Toolkit', 'juanjimeneztj' );
	...
	<div id="message" class="updated kt-plugin-install-notice-wrapper" style="position: relative; border:10px solid #fff; padding:10px 30px; background:#ebfbff;">
		...
		<p class="kt-notice-description"><?php /* translators: %s: <strong> */ printf( esc_html__( 'To take full advantage of the juanjimeneztj Theme please install the %1$sKadence Toolkit%2$s, this adds extra settings and features.', 'juanjimeneztj' ), '<strong>', '</strong>' ); ?></p>
		<p class="submit">
			...
			<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'juanjimeneztj-kadence-toolkit-plugin-notice', 'install' ), 'juanjimeneztj_toolkit_hide_notices_nonce', '_notice_nonce' ) ); ?>" class="notice-dismiss kt-close-notice"><span class="screen-reader-text"><?php esc_html_e( 'Skip', 'juanjimeneztj' ); ?></span></a>
		</p>
	</div>
	<?php
	wp_enqueue_script( 'kadence-toolkit-install' );
}
add_action( 'admin_notices', 'juanjimeneztj_kadence_toolkit_notice' );

/**
 * Hide Notice
 */
function juanjimeneztj_hide_toolkit_plugin_notice() {
	if ( isset( $_GET['juanjimeneztj-kadence-toolkit-plugin-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
 ...
 */
function juanjimeneztj_kadence_blocks_notice() {
	...
	if ( ! isset( $installed_plugins['kadence-blocks/kadence-blocks.php'] ) ) {
		$button_label = esc_html__( 'Install Kadence Blocks', 'juanjimeneztj' );
		$data_action  = 'install';
	} elseif ( ! juanjimeneztj_Plugin_Check::active_check( 'kadence-blocks/kadence-blocks.php' ) ) {
		$button_label = esc_html__( 'Activate Kadence Blocks', 'juanjimeneztj' );
		$data_action  = 'activate';
	} else {
		return;
	}
	...
	<div id="message" class="updated kt-plugin-install-notice-wrapper">
		<h3 class="kt-notice-title"><?php echo esc_html__( 'Thanks for choosing the juanjimeneztj Theme', 'juanjimeneztj' ); ?></h3>
		<p class="kt-notice-description"><?php /* translators: %s: <strong> <a> */ printf( esc_html__( 'We have a %1$snew plugin%2$s to extend and enhance the Block editor for your site. To take full advantage of the juanjimeneztj Theme please install the %3$sKadence Blocks%4$s, this adds extra editor blocks settings and features.', 'juanjimeneztj' ), '<a href="https://wordpress.org/plugins/kadence-blocks/" target="_blank">', '</a>', '<strong>', '</strong>' ); ?></p>
		<p class="submit">
			...
			<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'juanjimeneztj-kadence-blocks-plugin-notice', 'install' ), 'juanjimeneztj_blocks_hide_notices_nonce', '_notice_nonce' ) ); ?>" class="notice-dismiss kt-close-guten-notice"><span class="screen-reader-text"><?php esc_html_e( 'Skip', 'juanjimeneztj' ); ?></span></a>
		</p>
	</div>
	<?php
	wp_enqueue_script( 'kadence-toolkit-install' );
}
add_action( 'admin_notices', 'juanjimeneztj_kadence_blocks_notice' );

/**
 * Hide Notice
 */
function juanjimeneztj_hide_blocks_plugin_notice() {
	if ( isset( $_GET['juanjimeneztj-kadence-blocks-plugin-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
