...
class juanjimeneztj_custom_menu {
	...
	public static function instance() {
  	...
  	function __construct() {    
		...
		add_filter( 'wp_setup_nav_menu_item', array( $this, 'juanjimeneztj_add_nav_fields' ) );

		// add new fields via hook
		add_action( 'wp_nav_menu_item_custom_fields', array( $this, 'juanjimeneztj_custom_nav_fields' ), 10, 4 );

		// save menu custom fields
		add_action( 'wp_update_nav_menu_item', array( $this, 'juanjimeneztj_update_nav_fields'), 10, 3 );

		// edit menu walker
		add_filter( 'wp_edit_nav_menu_walker', array( $this, 'juanjimeneztj_edit_admin_walker') );
	...
	* @return      void
	*/
  	public function juanjimeneztj_add_nav_fields( $menu_item ) {
	...
	*/
  	public function juanjimeneztj_custom_nav_fields( $item_id, $item, $depth, $args ) {
		...
		<label for="edit-menu-item-ktlgmenu-<?php echo esc_attr($item_id); ?>">
		   <input type="checkbox" id="edit-menu-item-ktlgmenu-<?php echo esc_attr($item_id); ?>" value="enabled" name="menu-item-ktlgmenu[<?php echo esc_attr($item_id); ?>]"  <?php checked( $item->ktlgmenu, 'enabled' ); ?>  />
		    <?php _e( 'Enable Mega Sub-Menu', 'juanjimeneztj' ); ?>
		</label>
		</p>
		<p class="field-ktcolumnmenu description-wide ktcolumnmenu ktcolumnmenu-wide" style="clear:both">
		  <label for="edit-menu-item-ktcolumnmenu-<?php echo esc_attr($item_id); ?>">
		      <?php _e( 'Submenu Number of Columns', 'juanjimeneztj'); ?>
		...
		<p class="field-kticonmenu description description-thin">
		  <label for="edit-menu-item-kticonmenu-<?php echo esc_attr($item_id); ?>">
		      <?php _e( 'Icon Class (e.g. kt-icon-home)', 'juanjimeneztj' ); ?><br />
	...
	* @return      void
	*/
  	function juanjimeneztj_update_nav_fields( $menu_id, $menu_item_db_id, $args ) {
	...
	* @return      void
	*/
  	function juanjimeneztj_edit_admin_walker($walker) {
  
      	return 'juanjimeneztj_Walker_Nav_Menu_Custom';
      
  	}

}
function juanjimeneztj_custom_menu() {
	return juanjimeneztj_custom_menu::instance();
}

// instantiate plugin's class
$GLOBALS['juanjimeneztj_custom_menu'] = juanjimeneztj_custom_menu();